...
def send_message_text(sender_id,receiver_name,receiver_phone, text_message, in_out):
    data='''{
            "from":{ "type": "whatsapp", "number": "%s" },
            "to": { "type": "whatsapp", "number": "%s" },
            "message": {"content": 
            {"type": "text",
            "text": "%s"}}}'''%(sender,receiver_phone,text_message)
    ...
    sent_msg= Message(uuid,sender_id, sender,receiver_name,receiver_phone, in_out, "text", text_message, "None", "None","ON PROCESS")
...
def send_message_image(sender_id,receiver_name,receiver_phone, media_url, caption, in_out):
    data='''{
            "from":{ "type": "whatsapp", "number": "%s" },
            "to": { "type": "whatsapp", "number": "%s" },
            "message": {"content": 
            ...
            "image":{
                "url":"%s",
                "caption":"%s"}
            }}}'''%(sender,receiver_phone, media_url, caption)
      
    call_nexmo=requests.post(nexmo_host,headers=headers,data=data,auth=(secret_key,api_key))
     #Get the message_uuid or message id generated by NEXMO
    uuid=call_nexmo.json()['message_uuid']
    sent_msg= Message(uuid,sender_id,sender,receiver_name,receiver_phone,in_out,"image","None", media_url, caption,"ON PROCESS")
...
def send_message_file(sender_id,receiver_name,receiver_phone, media_url, caption, in_out):
    data='''{
            "from":{ "type": "whatsapp", "number": "%s" },
            "to": { "type": "whatsapp", "number": "%s" },
            "message": {"content": 
